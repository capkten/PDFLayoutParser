...
def batch_debug_directory(
    ...
    For each PDF, creates a dedicated parent directory named after the PDF stem,
    and saves stage-by-stage diagnostics (drawings, lines, cells, final tables)
    into structured subdirectories.
    """
    ...
    if not pdf_files:
        print(f"[pipeline_debug] No PDF files found in: {input_dir}")
        return []

    print(f"[pipeline_debug] Found {len(pdf_files)} PDF file(s) in: {input_dir}")
    os.makedirs(output_dir, exist_ok=True)

    generated_dirs: list[str] = []
    for pdf_file in pdf_files:
        pdf_stem = pdf_file.stem
        # Use PDF stem as parent directory for all sub-outputs
        pdf_out_dir = os.path.join(output_dir, pdf_stem)
        ...
        try:
            with PDFParser(str(pdf_file), render_dpi=dpi, use_ml=use_ml, debug_pipeline=True) as parser:
                ...
                if res.code == 1:
                    ...
                    generated_dirs.append(pdf_out_dir)
                else:
                    print(f"[pipeline_debug] Error parsing {pdf_file.name}: {res.message}")
        except Exception as exc:
            print(f"[pipeline_debug] Exception while processing {pdf_file.name}: {exc}")

    return generated_dirs


if __name__ == "__main__":
    batch_debug_directory(

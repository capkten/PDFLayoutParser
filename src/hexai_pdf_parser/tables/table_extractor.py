...
from hexai_pdf_parser.tables.table_config import TableConfig, LayoutProfile
from hexai_pdf_parser.tables.normalizers import (
    normalize_table_headers,
    normalize_complex_financial_header,
)
from hexai_pdf_parser.tables.table_profile_matcher import PageFeatures, match_profiles
from hexai_pdf_parser.tables.table_region_rules import (
    TableRegionCandidate,
    apply_region_rules as apply_region_rules_fn,
)
from hexai_pdf_parser.tables.table_structure_rules import apply_structure_rules
from hexai_pdf_parser.tables.table_rule_handlers import (
    get_region_handler,
    get_structure_handler,
)
from hexai_pdf_parser.debug.text_region_detector import (
    HorizontalSeparator,
    detect_candidate_regions,
    detect_separator_driven_regions,
)
from hexai_pdf_parser.tables.table_template_engine import (
    TemplateEngine,
    TableTemplateConfig,
    load_templates,
)
...
class TableExtractor:
    ...
    @staticmethod
    def _filter_contained_bboxes(items: List[Any], ios_threshold: float = 0.70) -> List[Any]:
        """Filter out redundant parent/enclosing boxes that contain multiple sub-tables."""
        if len(items) <= 1:
            return items

        def get_bbox(item: Any) -> BBox:
            return item[0] if isinstance(item, tuple) else item

        def box_area(b: BBox) -> float:
            return max(0.0, b.x1 - b.x0) * max(0.0, b.y1 - b.y0)

        def intersect_area(b1: BBox, b2: BBox) -> float:
            ix0 = max(b1.x0, b2.x0)
            iy0 = max(b1.y0, b2.y0)
            ix1 = min(b1.x1, b2.x1)
            iy1 = min(b1.y1, b2.y1)
        ...
        for i in range(n):
            if i in suppressed:
                continue
            b_parent = get_bbox(items[i])
            a_parent = box_area(b_parent)
            if a_parent <= 0:
                continue
            ...
            for j in range(n):
                if i == j or j in suppressed:
                    continue
                b_child = get_bbox(items[j])
                a_child = box_area(b_child)
                if a_child >= a_parent:
                    continue
                inter = intersect_area(b_parent, b_child)
                if a_child > 0 and inter / a_child >= ios_threshold:
                    contained_children.append(j)
    ...
    def _prune_empty_columns(
        ...
        """Prune phantom empty columns that have 0 non-empty text across all rows."""
    ...
    def _detect_columns_from_header_lines(
        self, page: Optional[fitz.Page], rows: List[dict], region_bbox: BBox
    ) -> List[tuple[float, float]]:
        """Detect column intervals directly from header underline drawing lines."""
        if page is None or not rows:
            return []
        try:
            drawings = page.get_drawings()
        except Exception:
        ...
        table_y1 = max(r["y1"] for r in rows)
        table_min_x = min(t["x0"] for r in rows for t in r["tokens"])
        table_max_x = max(t["x1"] for r in rows for t in r["tokens"])
            ...
            for it in d.get("items", []):
                if it[0] == "l":
                    ...
                    if abs(p1.y - p2.y) <= 1.5 and abs(p1.x - p2.x) >= 10.0:
                        y = p1.y
                        if table_y0 - 2.0 <= y <= table_y1 + 5.0:
        ...
        lines_by_y: dict[float, list[tuple[float, float]]] = {}
        ...
        merged_by_y: dict[float, list[tuple[float, float]]] = {}
            ...
            for s in segs:
                if not merged:
                    ...
                    if s[0] - merged[-1][1] <= 1.0:
                        ...
                        merged.append(list(s))
            valid = [tuple(m) for m in merged if (m[1] - m[0]) < (table_max_x - table_min_x) * 0.90]
            if valid:
                merged_by_y[y] = valid
        ...
        header_y_candidates = [
            y for y in merged_by_y.keys()
            if table_y0 + 2.0 <= y <= table_y0 + 60.0 and y <= table_y1 - 5.0
        ]
        if not header_y_candidates:
            header_y_candidates = [
                y for y in merged_by_y.keys()
                if table_y0 - 2.0 <= y <= table_y0 + 60.0 and y <= table_y1 - 5.0
            ]
        if not header_y_candidates:
            header_y_candidates = list(merged_by_y.keys())

        best_y = max(header_y_candidates, key=lambda y: (len(merged_by_y[y]), -y))
        line_cols = merged_by_y[best_y]
        line_cols.sort(key=lambda s: s[0])

        first_lx0 = line_cols[0][0]
        leading_tokens = [
            t for r in rows for t in r.get("tokens", [])
            if float(t.get("x1", 0)) <= first_lx0 + 5.0
        ]
        has_leading_unlined_col = (first_lx0 - table_min_x > 60.0 and len(leading_tokens) >= 2)

        cols: list[tuple[float, float]] = []
        curr_x = min(table_min_x - 5.0, region_bbox.x0)

        if has_leading_unlined_col:
            cols.append((curr_x, first_lx0))
            curr_x = first_lx0

        for i in range(len(line_cols)):
            lx0, lx1 = line_cols[i]
            if i + 1 < len(line_cols):
                nxt_lx0 = line_cols[i + 1][0]
                nxt_x = (lx1 + nxt_lx0) / 2.0
            else:
                nxt_x = max(table_max_x + 5.0, lx1 + 5.0, region_bbox.x1)

            cols.append((curr_x, nxt_x))
            curr_x = nxt_x
    ...
    def _build_text_grid_cells_from_boundaries(
        ...
        """Build grid cells using explicit column boundary x-coordinates and phrase grouping."""
        def _get_span(x0: float, x1: float) -> tuple[int, int]:
            if x1 < x0:
                x0, x1 = x1, x0
            start_col = 0
        ...
        cells: List[Cell] = []
        for row_idx, row in enumerate(rows):
            tokens = [t for t in row.get("tokens", []) if t.get("text", "").strip()]
            if not tokens:
                continue
            ...
            for phrase in phrases:
                px0 = min(t["x0"] for t in phrase)
                py0 = min(t["y0"] for t in phrase)
                px1 = max(t["x1"] for t in phrase)
                py1 = max(t["y1"] for t in phrase)
                start_col, end_col = _get_span(px0, px1)
                text = " ".join(t["text"].strip() for t in phrase if t["text"].strip()).strip()
                if not text:
                    continue
                cells.append(
                    Cell(
    ...
    def _build_text_grid_cells(
        ...
        # First pass: assign each token to column by left edge x0 to measure column extents
        col_tokens: dict[int, list[dict]] = {c: [] for c in range(num_cols)}
        for row in rows:
            for token in row["tokens"]:
                ...
                x0 = float(token["x0"])
                col = 0
                while col < len(start_thresholds) and x0 > start_thresholds[col]:
                    col += 1
                col = min(col, num_cols - 1)
                col_tokens[col].append(token)

        # Compute refined boundaries between adjacent columns
        boundaries = []
        for k in range(num_cols - 1):
            nxt_guide = guides[k + 1]
            k_tokens = [t for t in col_tokens[k] if float(t["x1"]) < nxt_guide + 10.0]
            max_x1 = max((float(t["x1"]) for t in k_tokens), default=guides[k])

            nxt_tokens = [t for t in col_tokens[k + 1] if float(t["x0"]) >= start_thresholds[k] - 5.0]
            min_x0 = min((float(t["x0"]) for t in nxt_tokens), default=nxt_guide)

            if max_x1 < min_x0:
                b = (max_x1 + min_x0) / 2.0
            else:
                b = (guides[k] + guides[k + 1]) / 2.0
            boundaries.append(b)
    ...
    def _infer_sparse_rowspans(
        ...
        If a horizontal line exists below the cell or between rows, rowspan is prohibited.
        """

        if not cells or not rows:
            return cells

        # Collect horizontal drawing lines if page is available
        h_lines: list[tuple[float, float, float]] = []
        if page is not None:
            try:
                for d in page.get_drawings():
                    for it in d.get("items", []):
                        if it[0] == "l":
                            p1, p2 = it[1], it[2]
                            if abs(p1.y - p2.y) <= 1.5 and abs(p1.x - p2.x) >= 5.0:
                                h_lines.append((p1.y, min(p1.x, p2.x), max(p1.x, p2.x)))
                        elif it[0] == "re":
                            r = it[1]
                            if r.height <= 3.0 and r.width >= 5.0:
    ...
    def _collect_text_rows(self, words: list[tuple]) -> list[dict]:
        ...
        for row in rows:
            row["tokens"] = self._merge_row_phrases(row["tokens"])

        return rows

    def _merge_row_phrases(
        self, tokens: list[dict], max_word_gap: float = 8.0
    ) -> list[dict]:
        """Merge horizontally close words/tokens in the same row into unified phrase tokens."""
        if len(tokens) < 2:
            return tokens
        ...
        for token in ordered[1:]:
            x_gap = token["x0"] - current["x1"]
            # Avoid merging two distinct numeric columns (e.g., adjacent amount columns)
            can_merge = (
                x_gap <= max_word_gap
                and not (current.get("is_numeric") and token.get("is_numeric") and x_gap > 3.0)
            )
            if can_merge:
                curr_txt = current["text"].rstrip()
                next_txt = token["text"].lstrip()
                sep = " " if not (curr_txt.endswith(("-", "/")) or next_txt.startswith(("-", "/"))) else ""
                current["text"] = curr_txt + sep + next_txt
                current["x1"] = max(current["x1"], token["x1"])
                current["y0"] = min(current["y0"], token["y0"])
                current["y1"] = max(current["y1"], token["y1"])
                current["y_center"] = (current["y0"] + current["y1"]) / 2
                current["is_numeric"] = self._classify_token_text(current["text"])["is_numeric"]
            else:
                merged.append(current)
                current = dict(token)
    ...
    def _merge_continuation_rows(
        self, rows: List[dict], page: Optional[fitz.Page] = None
    ) -> List[dict]:
        ...
        if page is not None:
            try:
                for d in page.get_drawings():
                    ...
                    rect = d.get("rect")
                    fill = d.get("fill")
                    if rect and rect.height <= 3.0 and rect.width >= 15.0 and fill in [(0.0, 0.0, 0.0), (0, 0, 0), None]:
        ...
        for i in range(1, len(rows)):
            ...
            prev_is_table_row = len(prev_row["tokens"]) >= 2
            effective_limit = gap_limit * 1.5 if prev_is_table_row else gap_limit

            if not row["tokens"]:
                continue

            text = row["tokens"][0].get("text", "").strip()
            # Do NOT merge standalone section headers (e.g. "Part I", "Part II", "Item 1", etc.)
            is_section_header = (
                text.startswith("Part ")
                or text.startswith("PART ")
            ...
            # Check if a horizontal line separates prev_row and row
            has_line_between = any(
                prev_row["y1"] - 3.0 <= ly <= row["y0"] + 3.0
                for ly, lx0, lx1 in h_lines
            )
            if has_line_between:
                continue
    ...
    def _extract_text_block_in_bbox(
        ...
        from hexai_pdf_parser.core.models import BBox, TextChar, TextBlock

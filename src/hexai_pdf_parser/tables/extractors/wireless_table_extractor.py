...
class WirelessTableExtractor(BaseTableExtractor):
    ...
    def _promote_grouped_header_cells(cells: List[Cell], columns: List[Tuple[float, float]], header_rows: int) -> None:
        ...
        for row_index in range(header_rows - 1):
            ...
            span_res = WirelessTableExtractor._infer_centered_parent_header_span(
                px0=title.bbox.x0,
                px1=title.bbox.x1,
    ...
    def extract(
        ...
        if page_language == "en":
            if table_bbox is None:
                # 检查页面是否存在多个独立的无线表格水平线段簇（如左右双栏排版各自包含独立表格）
                try:
                            ...
                            for cl in col_clusters:
                                if len(cl) >= 2:
                                    ...
                                    if related_words:
                                        ...
                                        t_res = self.extract_general_wireless(page, table_bbox=region_bbox, confidence=confidence)
                                        split_tables.extend(t_res)
                            if split_tables:
                                return split_tables
                except Exception:
                    pass
    ...
    def _detect_columns(
        ...
        if table_bbox is None:
            if words:
                ...
                table_bbox = BBox(page.rect.x0, page.rect.y0, page.rect.x1, page.rect.y1)
            else:
                table_bbox = BBox(0.0, 0.0, 612.0, 792.0)
        ...
        row_items_list = []
        for ry, rw in sorted(rows_by_y.items(), key=lambda item: item[0]):
            rw.sort(key=lambda w: w[0])
            items = []
            cur = []
            for w in rw:
            ...
            # 纯几何判别：一行中位于右侧（跳过最左侧行标签）且宽度较窄的离散块均为有效数据列候选项
            data_items = []
            for idx, it in enumerate(items):
                it_w = max(w[2] for w in it) - min(w[0] for w in it)
                it_mid = (min(w[0] for w in it) + max(w[2] for w in it)) / 2.0
                if idx > 0 and it_w <= (table_bbox.x1 - table_bbox.x0) * 0.35:
                    data_items.append(it)
                elif idx == 0 and len(items) >= 3 and it_w <= (table_bbox.x1 - table_bbox.x0) * 0.35:
                    # 全数据行（如纯数据无行标签）
                    data_items.append(it)
            row_items_list.append((ry, items, len(data_items)))

        dollar_x0s = [float(w[0]) for w in table_words if "$" in str(w[4])]

        max_d_items = max((d_count for _, _, d_count in row_items_list), default=0)

        candidate_cuts = []
        for ry, items, d_count in row_items_list:
            if d_count >= max(2, max_d_items - 2) and len(items) >= 2:
                valid_items = [it for it in items if (max(w[2] for w in it) - min(w[0] for w in it)) <= (table_bbox.x1 - table_bbox.x0) * 0.35]
                if len(valid_items) >= 2:
                    for k in range(len(valid_items) - 1):
                        left_it = valid_items[k]
                        right_it = valid_items[k + 1]
                        left_x1 = max(w[2] for w in left_it)
                        right_x0 = min(w[0] for w in right_it)
                        matched_dollar = next((dx for dx in dollar_x0s if abs(right_x0 - dx) <= 35.0 or (left_x1 < dx <= right_x0)), None)
                        if matched_dollar is not None:
                            candidate_cuts.append(matched_dollar - 1.0)
                        elif left_x1 < right_x0:
                            candidate_cuts.append((left_x1 + right_x0) / 2.0)
                        else:
                            candidate_cuts.append(left_x1 + 1.0)

        sorted_cuts = sorted(candidate_cuts)
        fused_cuts = []
        for c in sorted_cuts:
            if not fused_cuts:
                fused_cuts.append([c])
            else:
                if c - sum(fused_cuts[-1]) / len(fused_cuts[-1]) <= 6.0:
                    fused_cuts[-1].append(c)
                else:
                    fused_cuts.append([c])

        final_cuts = [sum(group) / len(group) for group in fused_cuts]
        ...
        boundaries = sorted(list(set(final_cuts)))
        columns = []
        curr_x = table_x0
        for b in boundaries:
            if b > curr_x + 3.0:

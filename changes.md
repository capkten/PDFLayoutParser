...
- 修复 `fix/zh_all_table_pages.pdf` 页面索引 `938` 上方“开发成本”中文无线表格因跨 native line 的日期片段产生占位冲突而整表丢失的问题。
  ...
  - **测试与页面验证**：新增跨行中文后缀正例、不同 block 独立数值反例及 Page 938 no-words 集成测试；无线结构与相关页面专项共 `221 passed`。使用当前代码独立重跑页面索引 `938` 至 `D:\codes\PDFLayoutParser\output\page_938_wrapped_numeric_suffix_20260903\`：两张 `wireless_span_recovery` 表分别为 `7x7`、`3x7`，逻辑槽位覆盖 `49/49`、`21/21`，occupancy conflict 均为 0；最终 PNG 为 `tables\page-938.png`，视觉复核确认两表边界独立，无误并、越列或明显线框错位。

- 修复页面索引 `591` 中文无线表格因跨行前置表头导致的整表漏检。模型正确检出 `[55.6,120.2,786.6,508.1]`，native-span 输出模式为 `columnar`，但 `rescue_header_only_leaf_bands()` 要求最低表头层覆盖全部稳定列带；本页“项目/附注”位于上一层，最低层只覆盖金额列后缀，导致“其他权益工具”“资本公积”“减：库存股”“专项储备”未形成列带，表头挤槽后触发 `R2C3/R2C4` occupancy conflict，恢复器返回 0 表。现在仅新增“连续稳定后缀 + 上一层逐列唯一覆盖前置结构列 + 候选叶子整组无重叠且间距明确”的事务性补充分支；完整覆盖旧分支、冲突检查和空槽位物化保持不变。调用位于 `src/hexai_pdf_parser/tables/wireless_structure/header_topology.py` 的 `rescue_header_only_leaf_bands()`，接入位置仍为 `recoverer.py` 列标注之前；全程只消费 native span、atom、列带和逻辑网格，不回读 `page.get_text("words")`，不进入 zebra/legacy。新增 page-591 形态正例，以及中间缺列、上层证据不足、候选互相重叠反例，并锁定旧完整覆盖分支的增量过滤与数值候选行为。相关无线结构测试 `123 passed`；扩展回归 `232 passed, 1 failed`，唯一失败为工作区已有的 `test_hybrid_wired_table_replaces_full_rowspan_body_before_shifting_footer`（与本次改动无关，当前实际 `line_projection`、预期 `hybrid_line_span_recovery`）。使用最终代码独立重跑 `fix/zh_all_table_pages.pdf` 页索引 `591` 至 `D:\codes\PDFLayoutParser\output\page_591_rowspan_prefix_leaf_recovery_20260903\`：1 张 `wireless_span_recovery` 表，`29x11`，309 个 Cell，逻辑槽位 `319/319` 唯一占用且无冲突；结构化结果为 `pages\page-591.json`，表格 PNG 为 `tables\page-591.png`。

- 优化中文/混合表格主体恢复（`hybrid_line_span_recovery`）的结构支持度判定，防止大段叙述性文本单元格（如 `fix/zh_all_table_pages.pdf` 页面索引 `200` 关键审计事项）被误判为多行数据网格并错误切碎。
  - **根因与调用位置**：
    1. 在 `src/hexai_pdf_parser/tables/table_extractor.py` 的 `_recover_hybrid_wired_table()` 中，原先仅依据 `heights[body_row] >= max(60.0, 3.0 * other_heights)` 的纯几何行高阈值决定是否尝试混合重构。当有线表格包含审计报告“关键审计事项”（如 Page 200“存货减值”包含长达 20 行的“事项描述”与 5 条详细“审计应对”）等大段叙述文本时，行高高达 420.5pt，远超表头行（20.5pt/80.6pt），被无差别送入 `recover_hybrid_body_cells()`。
    2. 在 `src/hexai_pdf_parser/tables/wireless_structure/hybrid_body.py` 的 `_has_hybrid_structure_support()` 中，原先仅校验 `multi_support_rows >= 2` 或 `multi_support_rows >= 1 and cols_with_multiple_rows >= 2`。左列长达 394pt 的大段落被 `build_grid` 依据其几何中心（`center_y = 394.2`）误对齐到右列第 3 条措施行，导致系统误判第 1 行与第 4 行同时具备双列内容（`multi_support_rows = 2`），并在左列生硬填入 4 个空白单元格，将 3x2 有线表切碎为 8x2 伪多行表。
  - **修复判定与调用位置**：
    1. 在 `src/hexai_pdf_parser/tables/wireless_structure/hybrid_body.py` 的 `_has_hybrid_structure_support()` 中，引入独立数据项与段落特征守卫：
       - **数值列支持优先**：检查是否存在多行独立存在的数值/金额列（`_is_numeric_item`，如数字、百分比或标准占位符 `-`、`——`、`/`）；若存在数值列支撑（如 Page 371、Page 914、Page 916），保持既有混合恢复通道。
       - **长篇连续叙述段落拦截**：若全表缺乏独立数值列支撑，且存在长篇连续段落文本（`_is_narrative_paragraph`，字符数 $\ge 30$ 或包含分号/句号/冒号的大段文本），直接拒绝多行结构支持，保持原始有线线框。
       - **稀疏大段落列拦截**：若全表缺乏独立数值列支撑，且某列存在大面积空白槽位（非空率 $< 60\%$）或行支持率不足 $60\%$，坚决拒绝判定为多行混合结构。
  - **结构约束**：全流程严格基于 native span、atom、列带与逻辑网格拓扑决策，不回读 `page.get_text("words")`，不回退 zebra 或 legacy 路径，严格遵循 0 Occupancy Conflict 契约与严格空槽位物化。
  - **测试与页面验证**：新增 `tests/test_hybrid_narrative_guard.py`，覆盖 Page 200 审计叙述段落拒绝混合切分正例、稀疏长文本列拒绝反例、真实财务报表混合主体通过正例（`3 passed`）；既有混合测试 `tests/test_hybrid_body_recovery.py` 为 `8 passed`，有线/无线混合集合共 `88 passed`。页面索引 `200` 独立重跑至 `D:\codes\PDFLayoutParser\output\fix_page_200_narrative_guard_20260903\`：表格保持原始有线结构（`line_projection`，`3x2`，5 个 Cell 完整覆盖），无伪多行切分与空单元格错位；可视化 PNG `tables/page-200.png` 经视觉核验，左侧存货减值描述与右侧 5 条审计应对完整贴合单元格；真实财务混合表页面（Page 371 `11x3` 10个数值Cell、Page 914 `41x6` 84个数值Cell、Page 916 `54x6` 108个数值Cell）均保持 `hybrid_line_span_recovery`，零回归。
